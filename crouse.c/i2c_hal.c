...
static int i2c_wait(I2C_TypeDef *i2c, uint32_t flag, uint32_t timeout_ms)
{
    ...
    while ((i2c->SR1 & flag) == 0) {
        ...
        if (i2c->SR1 & (I2C_SR1_AF | I2C_SR1_BERR | I2C_SR1_ARLO))
            return -1;
        if ((Systick_GetTick() - start) > timeout_ms) return -1;
    }
    return 0;
}
...
/* ================================================================
 * 公开 API — I2C1 (PB8=SCL, PB9=SDA — 重映射)
 * ================================================================ */
...
int I2C2_ReadReg(uint16_t dev_addr, uint8_t reg, uint8_t *data, uint16_t len)
{
    return i2c_xfer(I2C2, g_i2c2_mutex, dev_addr, reg, data, len, 1);
}
